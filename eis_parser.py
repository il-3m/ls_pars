...
def _looks_like_qty_text(text: str) -> bool:
    ...
    if re.search(r"\b(ШТУКА\s*\(ШТ\)|ГРАММ\s*\(Г\)|МИЛЛИГРАММ\s*\(МГ\)|КУБИЧЕСКИЙ\s+САНТИМЕТР[^|]*\(СМ\[?3\*?\][^)]*\)|МИЛЛИЛИТР\s*\(МЛ\)|ШТ\.?|СМ\[?3\*?\])\b", src, flags=re.IGNORECASE):
        return True
    # Проверка на единицы измерения:
    # - точка допускается только как десятичный разделитель между цифрами (2.5 мл)
    # - после числа должен быть пробел перед единицей измерения
    # - это предотвращает ложные совпадения типа "4. Ме" (начало названия препарата)
    return bool(re.search(r"\d[\d\s,]*(?:\.\d+)?\s+(СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b", src, flags=re.IGNORECASE))
...
def _extract_qty_from_blob(text: str) -> str:
    ...
    if labeled:
        ...
        qty_pattern = re.search(r"\d[\d\s,]*(?:\.\d+)?\s+(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b(?!\s*/\s*(?:МЛ|МГ|Г|МКГ))", value, flags=re.IGNORECASE)
        ...
        if re.search(r"количество", value, flags=re.IGNORECASE) and not qty_pattern:
            return ""
        # Дополнительная проверка: если value содержит только единицы измерения (СМ3; МЛ) и информацию об упаковках,
        # но не содержит фактического числового значения объема - возвращаем пустую строку
        # Проверяем, что это просто перечисление единиц без числа перед ними
        if re.search(r"^\s*(?:КУБИЧЕСКИЙ\s+САНТИМЕТР|СМ3|МЛ|МИЛЛИЛИТР)[^|]*$", value, flags=re.IGNORECASE):
            return ""
        # Если value содержит только "СМ3; МЛ" и информацию о количестве упаковок, но нет числа с единицей объема
        if qty_pattern is None:
            return ""
        # Если найден числовой объем, извлекаем только его (до слов "количество")
        qty_match = re.search(r"(\d[\d\s,]*(?:\.\d+)?\s+(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b)(?!\s*/\s*(?:МЛ|МГ|Г|МКГ))", value, flags=re.IGNORECASE)
        if qty_match:
            return _clean(qty_match.group(1))
        return value
    ...
    m = re.search(r"(\d[\d\s,]*(?:\.\d+)?\s+(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b[^|]*)", src, flags=re.IGNORECASE)
    return _clean(m.group(1)) if m else ""
...
() => {
  ...
  const looksLikeQty = (s) => {
    const src = clean(s);
    // Проверка на единицы измерения с обязательными границами слов
    // МЕ должно быть отдельным словом, а не частью другого слова (например, "Метамизол")
    return /\d[\d\s.,]*\s*(СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b/i.test(src)
  ...
  const extractQty = (srcText) => {
    ...
    if (labeled) {
      let value = clean(labeled[1].split(/СТРАНА\s+ПРОИСХОЖДЕНИЯ|ПРЕПАРАТЫ|ТОВАР\b|НАИМЕНОВАНИЕ\s+ДЕРЖАТЕЛЯ|НАИМЕНОВАНИЕ\s+ПРОИЗВОДИТЕЛЯ/i)[0]);
      // Проверка: если в значении нет числового количества (только единицы измерения и описания упаковок),
      // то это не объем, а описание единицы измерения - возвращаем пустую строку
      // Ищем паттерн числа с возможными единицами измерения (мл, см3, шт, ед, г, кг, л и т.д.)
      // Важно: исключаем паттерны дозировок типа "10 мг/мл" или "10мг/мл"
      // Точка допускается только как десятичный разделитель между цифрами
      const qtyPattern = value.match(/\d[\d\s,]*(?:\.\d+)?\s+(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b(?!\s*\/\s*(?:МЛ|МГ|Г|МКГ))/i);
      // Если есть слово "количество" и нет числового объема с единицами измерения - это описание упаковки
      if (/количество/i.test(value) && !qtyPattern) {
        return '';
      }
      // Дополнительная проверка: если value содержит только единицы измерения (СМ3; МЛ) и информацию об упаковках,
      // но не содержит фактического числового значения объема - возвращаем пустую строку
      if (/^\s*(?:КУБИЧЕСКИЙ\s+САНТИМЕТР|СМ3|МЛ|МИЛЛИЛИТР)[^|]*$/i.test(value)) {
        return '';
      }
      // Если найден числовой объем, извлекаем только его (до слов "количество")
      const qtyMatch = value.match(/(\d[\d\s,]*(?:\.\d+)?\s+(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b)(?!\s*\/\s*(?:МЛ|МГ|Г|МКГ))/i);
      if (qtyMatch) {
        return clean(qtyMatch[1]);
      }
      return value;
    }
    ...
    const m = src.match(/\d[\d\s,]*(?:\.\d+)?\s+(СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)\b(?:\s*\([^)]*\))?/i);
    return m ? clean(m[0]) : '';
  };
