...
def _extract_qty_from_blob(text: str) -> str:
    ...
    if labeled:
        ...
        qty_match = re.search(r"(\d[\d\s.,]*\s*(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ))(?!\s*/\s*(?:МЛ|МГ|Г|МКГ))", value, flags=re.IGNORECASE)
        if qty_match:
...
() => {
  ...
  const extractQty = (srcText) => {
    ...
    if (labeled) {
      ...
      // Ищем паттерн числа с возможными единицами измерения (мл, см3, шт, ед, г, кг, л и т.д.)
      // Важно: исключаем паттерны дозировок типа "10 мг/мл" или "10мг/мл"
      const qtyPattern = value.match(/\d[\d\s.,]*\s*(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ)(?!\s*\/\s*(?:МЛ|МГ|Г|МКГ))/i);
      // Если есть слово "количество" и нет числового объема с единицами измерения - это описание упаковки
      ...
      // Если найден числовой объем, извлекаем только его (до слов "количество")
      const qtyMatch = value.match(/(\d[\d\s.,]*\s*(?:СМ3|МЛ|Л|Г|КГ|ШТ|ЕД|МЕ))(?!\s*\/\s*(?:МЛ|МГ|Г|МКГ))/i);
      if (qtyMatch) {
