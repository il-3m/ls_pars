...
test_cases = [
    ...
    "10 МГ/МЛ+1 МГ/МЛ",        # Только дозировка (без количества) - ТЕПЕРЬ НЕ ДОЛЖНО ЛОМАТЬСЯ
]

# Паттерн для одного значения дозировки
single_dose = r'(\d+(?:[.,]\d+)?)\s*(мг|мл|мкг|г|ед|mg|ml|mcg|g)(?:\s*/\s*(мг|мл|мкг|г|ед|mg|ml|mcg|g))?'

print("=== ТЕСТИРОВАНИЕ ПАРСИНГА ДОЗИРОВКИ (ИСПРАВЛЕНИЕ 2) ===\n")
...
for dose_cell_text in test_cases:
    ...
    # Разбиваем слипшиеся цифры в начале строки - НОВАЯ ЛОГИКА
    if re.match(r'^([1-9])([1-9]\d*)', dose_cell_text):
        dose_cell_text = re.sub(r'^([1-9])([1-9]\d*)', r'\1 \2', dose_cell_text)
        print(f"После разбивки: {repr(dose_cell_text)}")
    else:
        print("Разбивка не применялась")
